...
function checkEnv() {
  ...
  if (!process.env.BREVO_WHATSAPP_SENDER) {
    warnings.push("WhatsApp notifications are not configured (BREVO_WHATSAPP_SENDER, using the same BREVO_API_KEY as SMS) — turning on WhatsApp in Announcements settings will just log instead of sending.");
  }
