...
- **SMS**: Celcom Africa (`CELCOM_API_KEY`/`CELCOM_PARTNER_ID`/`CELCOM_SHORTCODE`
  in `.env`), reusing the same phone-normalization already used for
  M-Pesa. Celcom's API natively accepts a comma-separated recipient list,
  so publishing an announcement is genuinely one HTTP call for the whole
  matching-audience batch (chunked at 500 recipients per request as a
  defensive cap) — respecting the Enable SMS toggle.
- **WhatsApp**: Brevo Transactional WhatsApp API
  (`BREVO_WHATSAPP_SENDER`, plus `BREVO_API_KEY`) — see
...
- Needs `BREVO_API_KEY` configured (see Email notifications above) — without it,
...
| Notification Channels / Auto Messages / Templates (Announcements) | Real email (Brevo), SMS (Celcom Africa), and WhatsApp (Brevo) sent on registration, payment, admission, announcement publish, and certificate issuance |
